...
class CouponController extends Controller
{
    ...
    public function __construct(Coupon $coupon, CouponCustomer $coupon_customer, Discount $discount, DiscountType $discountType, Cart $cart, Service $service, Booking $booking)
    {
    ...
    public function apply_coupon(Request $request): JsonResponse
    {
        ...
        if ($coupon->coupon_type == 'first_booking') {
            $bookings = $this->booking->where('customer_id', $request->user()->id)->count();
            if ($bookings > 1) {
